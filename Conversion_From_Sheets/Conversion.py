...
##Importing all of the needed packages to run this program 
...
#This next section opens a tkinter window which allows for selection of the materials used in the spreadsheets.
...
#This little method defines an ok button which can close the window
...
################
#The following section allows for the input of the temperature range needed and allows the user to specify whether they want graphs or tables
########## 

#Labelling the windows boxes
master1 = Tk()
...
#Snapping the boxes to a grid within the window
ltemp.grid(row=1, column=1)
htemp.grid(row=2, column=1)
step.grid(row=3, column=1)

#defining a button to close this window
def ok1():

    master1.quit()
 

#Checkbuttons for user input if they want graphs or tables  
var1 = IntVar()
...
#The inputted temperature range is then made into an actual list which can be used.

TempK = range(int(ltemp.get()),int(htemp.get())+int(step.get()),int(step.get()))


######
#This next section allows for the user to specify whether they're inputting P&T or V&T using a drop-down menu
######
...
#defining an ok button which can close the window
def ok6():
    presorvol.quit()

#this makes the drop down menu that allows for the selection of pressure or volume
...
#If the input is volume, then the program asks the user for the file which contains the volumes (These are given in angstroms^3 per unit cell)
...
#If the input is pressure then the program asks the user for the file which contains the pressures (These need to be in GPa)
...
#This while loop allows the program to decide what material has been selected.
...
#The following block of code initialises empty arrays for holding all the data in the spreadsheet where 'listshape' is defined earlier
...
#If custom inputs are necessary then the parameters need to be introduced - This is done usingthe following 'if' statement
#This again uses tkinter to create a gui
...
if custtest == 1:
	...
	#This ok button closes the parameter set window if needed
	def ok2():
	...
	#This sets the button to be in a certain place in the window
	Button(master2, text='OK', command=ok).grid(row=20, column=0, sticky=E, pady=4)


	#This runs the tkinter GUI
	master2.mainloop()
	
	#This appends the values to a list
...
#Deciding whethere we have V-T or P-T
if inputatt.get()=='Volume':
	attribute = vol
elif inputatt.get() =='Pressure':
	attribute = press
...
#This next section calculates all the values for the given volumes in the spreadsheets. Easier to implement this way due to the cross linking of equations
...
##########
#This section is where a window pops up and allows the user to select what attribute they are interested in - All attributes from the spreadsheets are copied here
##########
master3 = Tk()
...
#creating an ok button which closes the tkinter window
def ok():
...
#sticking the ok button to the tkinter window
...
###
#The following set of if statements explains fully the slightly complicated set of lists outlined above
# There needs to be a link created between the list and the value selected in the drop down menu
###
outval = []
...
###This final section allows for plotting/saving of the graph and showing/saving of the dataset if so requested by the user.
